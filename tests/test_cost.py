...
from tests.test_reasoning import DONE, PROMPT, _delta_frame, _frame, _raw_client, _settings
...
async def test_missing_usage_chunk_leaves_usage_none() -> None:
    ...
    client_two = ReasoningClient(_settings(), http_client=_raw_client(body_no_done, bodies))
...
async def test_usage_populated_after_include_usage_drain() -> None:
    ...
    client_two = ReasoningClient(_settings(), http_client=_raw_client(body_no_details, bodies))
    stream_two = client_two.start_turn(PROMPT)
    assert [chunk async for chunk in stream_two]
    await client_two.aclose()
